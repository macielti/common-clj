...
(s/deftest consume-update!-test
  (testing "that we can consume a update"
    (component.telegram.core/consume-update! update consumers {:telegram {:token token}})
    (is (= "/test testing"
           @test-state))
    (reset! test-state nil))
  (testing "that we can handle exception with error-handler provided by the user of the component"
    (component.telegram.core/consume-update! update-with-exception-in-command-consumption consumers {:telegram {:token token}})
    (is (= {:cause :nothing}
           @test-state))
    (reset! test-state nil))
    ...
    (fake/with-fake-routes
      {(format "https://api.telegram.org/bot%s/sendMessage" token) (fn [{:keys [body] :as request}]
                                                                     (reset! test-state (-> (slurp body)
                                                                                            ...
                                                                                            :text))
                                                                     {})}
      (component.telegram.core/consume-update! update-with-unmatched-command-message consumers {:telegram telegram}))
